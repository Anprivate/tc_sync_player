...
type
  ...
  TAudioTCReader = class(TComponent, ISampleGrabberCB)
  private
...
procedure TAudioTCReader.AudioDataToTC;
var
  i, i1, i2: integer;
  SampleValue: integer;
...
procedure TAudioTCReader.DeInit;
begin
  if UseSdiIn then
    DeInitSDIDevice()
  else
...
procedure TAudioTCReader.DeInitAudioDevice;
var
  i: integer;
begin
  WaveInStop(WaveIn);
...
procedure TAudioTCReader.DeInitSDIDevice;
var
  hr: HResult;
  pEnum: IEnumFilters;
  pFilter: IBaseFilter;
...
destructor TAudioTCReader.Destroy;
begin
  inherited;
end;
...
begin
  ...
  if TAudioTCReader(dwInstance).AudioDataCntr = 0 then
  begin
    Move(PWaveHdr(dwParam1)^.lpData^, TAudioTCReader(dwInstance).AudioData^,
      bufsize * 4);
    TAudioTCReader(dwInstance).AudioDataCntr := bufsize;
  end;
  ...
  _DiffMode := TAudioTCReader(dwInstance).DiffMode;
  if _DiffMode or (TAudioTCReader(dwInstance).UseChannel = 0) then
    i1 := 0
  else
    i1 := 1;
  ...
  TAudioTCReader(dwInstance).AudioDataToTC();
end;

procedure TAudioTCReader.Init;
begin
...
procedure TAudioTCReader.InitAudioDevice;
var
  i: integer;
  InDevNum: Word;
...
procedure TAudioTCReader.InitSDIDevice;
var
  i: integer;
  hr: HResult;
...
begin
  ...
  case Card_no of
    1:
      begin
        DLV_CLSID := CLSID_DecklinkVideoCaptureFilter;
        DLA_CLSID := CLSID_DecklinkAudioCaptureFilter;
      end;
    2:
      begin
        DLV_CLSID := CLSID_DecklinkVideoCaptureFilter2;
        DLA_CLSID := CLSID_DecklinkAudioCaptureFilter2;
      end;
    3:
      begin
        DLV_CLSID := CLSID_DecklinkVideoCaptureFilter3;
        DLA_CLSID := CLSID_DecklinkAudioCaptureFilter3;
      end;
    4:
      begin
        DLV_CLSID := CLSID_DecklinkVideoCaptureFilter4;
        DLA_CLSID := CLSID_DecklinkAudioCaptureFilter4;
      end;
    5:
      begin
        DLV_CLSID := CLSID_DecklinkVideoCaptureFilter5;
        DLA_CLSID := CLSID_DecklinkAudioCaptureFilter5;
      end;
    6:
      begin
        DLV_CLSID := CLSID_DecklinkVideoCaptureFilter6;
        DLA_CLSID := CLSID_DecklinkAudioCaptureFilter6;
      end;
    7:
      begin
        DLV_CLSID := CLSID_DecklinkVideoCaptureFilter7;
        DLA_CLSID := CLSID_DecklinkAudioCaptureFilter7;
      end;
    8:
      begin
        DLV_CLSID := CLSID_DecklinkVideoCaptureFilter8;
        DLA_CLSID := CLSID_DecklinkAudioCaptureFilter8;
      end;
  else
  ...
  try
    ...
    CaptureName := VendorInfo + ' (' + inttostr(Card_no) + ')';
    DeviceName := CaptureName;
    ...
    if CapWidth > 720 then
      NeededSubType := IID_MEDIASUBTYPE_HDYC
    else
    ...
    // set call back
    hr := SampleGrabberAudio.SetCallback(Self, 0);
    if FAILED(hr) then
    begin
      AMGetErrorText(hr, PChar(ErrMsg), 512);
      raise EMyOwnException.Create(Trim(ErrMsg));
    ...
    ConnectPins(DLVoutpin, RenderPin,
      'DeckLink video out -> VMR Renderer in connected',
      'DeckLink video out -> VMR Renderer in failed - ');
...
function TAudioTCReader.BufferCB(SampleTime: Double; pBuffer: PByte;
  BufferLen: integer): HResult;
begin
  Result := S_OK;
end;

constructor TAudioTCReader.Create(AOwner: TComponent);
begin
  inherited;
...
function TAudioTCReader.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := inherited QueryInterface(IID, Obj);
end;

function TAudioTCReader.SampleCB(SampleTime: Double;
...
begin
  ...
  if sample_size <> (bufsize * 2 * 16) then
    exit;
  ...
  if AudioDataCntr = 0 then
  begin
    Move(data16^, _SDIAudioData^, bufsize * 4);
    AudioDataCntr := bufsize;
  end;
...
function TAudioTCReader._AddRef: integer;
begin
  Result := 2;
end;

function TAudioTCReader._Release: integer;
begin
  Result := 1;
end;
